    ...
    namespace orientation {
        ...
        struct CostItems {
            ...
            std::string field_values() {
                ...
                ss << overhang << ",\t" << bottom << ",\t" << bottom_hull << ",\t" << contour
                   << ",\t" << area_laf << ",\t" << area_projected << ",\t" << unprintability;
                return ss.str();
            }
        };
...
// management and spatial index structures for acceleration.
        ...
        public:
            AutoOrienter(OrientMesh *orient_mesh_,
                ...
                params.ASCENT = cos(PI - orient_mesh->overhang_angle * PI /
                                         180); // use per-object overhang angle
            ...
            struct VecHash {
                size_t operator()(const Vec3f &n1) const {
                    return std::hash<coord_t>()(int(n1(0) * 100 + 100)) +
                           std::hash<coord_t>()(int(n1(1) * 100 + 100)) * 101 +
                           std::hash<coord_t>()(int(n1(2) * 100 + 100)) * 10221;
                }
            };

            Vec3f quantize_vec3f(const Vec3f n1) {
                return Vec3f(floor(n1(0) * 1000) / 1000, floor(n1(1) * 1000) / 1000,
                             floor(n1(2) * 1000) / 1000);
            }
            ...
            Vec3d process() {
                ...
                for (int i = 0; i < orientations.size(); i++) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << std::fixed << std::setprecision(4) << "orientation:"
                                            << orientation.transpose() << ", cost:" << std::fixed
                                            << std::setprecision(4) << cost_items.field_values();
                    std::cout << std::fixed << std::setprecision(4) << "orientation:"
                              << orientation.transpose() << ", cost:" << std::fixed
                ...
                sort(results_vector.begin(), results_vector.end(),
                     [](const PAIR &p1, const PAIR &p2) {
                         return p1.second.unprintability < p2.second.unprintability;
                     });
                ...
                BOOST_LOG_TRIVIAL(info) << std::fixed << std::setprecision(6) << "best:"
                                        ...
                                        << results_vector[0].second.field_values();
            ...
            void preprocess() {
                int count_apperance = 0;
                {
                    ...
                    for (size_t i = 0; i < face_count; i++) {
                        float area = get_its_facet_area(its, i);
                ...
                if (orient_mesh)
                    BOOST_LOG_TRIVIAL(debug) << orient_mesh->name << ", count_apperance="
                                             << count_apperance;
            ...
            void area_cumulation(const Eigen::MatrixXf &normals_, const Eigen::VectorXf &areas_,
                ...
                for (size_t i = 0; i < num_directions; i++) {
                    ...
                    BOOST_LOG_TRIVIAL(debug) << align_counts[i].first.transpose() << ", area: "
                                             << align_counts[i].second;
                }
            }
            ...
            void area_cumulation_accurate(std::vector<Vec3f> &normals_,
                                          ...
                                          const Eigen::VectorXf &areas_, int num_directions = 10) {
                ...
                for (auto i = 0; i < areas_.size(); i++) {
                    alignments_.insert(
                            std::pair(quantize_normals_.row(i), std::pair(current_areas, n1)));
                }
                // cumulate areas
                for (auto i = 0; i < areas_.size(); i++) {
                ...
                sort(align_counts.begin(), align_counts.end(), [](const PAIR &p1, const PAIR &p2) {
                    return p1.second.first[1] > p2.second.first[1];
                });

                num_directions = std::min((size_t) num_directions, align_counts.size());
                for (size_t i = 0; i < num_directions; i++) {
                    orientations.push_back(align_counts[i].second.second);
                    BOOST_LOG_TRIVIAL(debug) << align_counts[i].second.second.transpose()
                                             << ", area: " << align_counts[i].second.first[1];
                }
            }
            ...
            void project_vertices(Vec3f orientation) {
                ...
                for (size_t i = 0; i < face_count; i++) {
                    float z0 = get_its_vertex(its, i, 0).dot(orientation);
            ...
            argsort(const Eigen::VectorXf &vec, std::string order = "ascend") {
                Eigen::VectorXi ind = Eigen::VectorXi::LinSpaced(vec.size(), 0,
            ...
            CostItems get_features(Vec3f orientation, bool min_volume = true) {
                ...
                // filter bottom area
                auto bottom_condition =
                        z_max.array() < total_min_z + this->params.FIRST_LAY_H - EPSILON;
                auto bottom_condition_hull =
                        z_max_hull.array() < total_min_z + this->params.FIRST_LAY_H - EPSILON;
                auto bottom_condition_2nd =
                        z_max.array() < total_min_z + this->params.FIRST_LAY_H / 2.f - EPSILON;
                ...
                costs.bottom = bottom_condition.select(areas, 0).sum() * 0.5 +
                               bottom_condition_2nd.select(areas, 0).sum();

                // filter overhang
                Eigen::VectorXf normal_projection(normals.rows(),
                                                  1);// = this->normals.dot(orientation);
                for (auto i = 0; i < normals.rows(); i++) {
                    normal_projection(i) = normals.row(i).dot(orientation);
                }
                auto areas_appearance = areas.cwiseProduct(
                        (is_apperance * params.APPERANCE_FACE_SUPP +
                         Eigen::VectorXf::Ones(is_apperance.rows(), is_apperance.cols())));
                auto overhang_areas = ((normal_projection.array() < params.ASCENT) *
                                       (!bottom_condition_2nd)).select(areas_appearance, 0);
                ...
                if (min_volume) {
                    ...
                    costs.overhang = (heights.array() * overhang_areas.array() *
                                      inner.array()).sum();
                } else {
                ...
                Eigen::MatrixXf laf_areas = ((normal_projection_abs.array() < params.LAF_MAX) *
                                             (normal_projection_abs.array() > params.LAF_MIN) *
                                             (z_max.array() >
                                              total_min_z + params.FIRST_LAY_H)).select(areas, 0);
            ...
            float target_function(CostItems &costs, bool min_volume) {
                ...
                if (min_volume) {
                    ...
                    cost = params.TAR_A * (overhang + params.TAR_B) + params.RELATIVE_F *
                                                                      (/*costs.volume/100*/
                                                                              overhang *
                                                                              params.TAR_C +
                                                                              params.TAR_D +
                                                                      ...
                                                                      (params.TAR_D +
                                                                       ...
                                                                       params.BOTTOM_HULL_F *
                                                                       bottom_hull +
                                                                       params.TAR_E * overhang +
                                                                       params.TAR_PROJ_AREA *
                                                                       costs.area_projected);
                } else {
                    float overhang = costs.overhang;
                    cost = params.RELATIVE_F * (costs.overhang * params.TAR_C + params.TAR_D +
                                                params.TAR_LAF * costs.area_laf *
                                                params.use_low_angle_face) /
                           (params.TAR_D + params.CONTOUR_F * costs.contour +
                            params.BOTTOM_F * bottom + params.BOTTOM_HULL_F * bottom_hull +
                            params.TAR_PROJ_AREA * costs.area_projected);
                }
                cost += (costs.bottom < params.BOTTOM_MIN) *
                        100;// +(costs.height_to_bottom_hull_ratio > params.height_to_bottom_hull_ratio_MIN) * 110;
        ...
        void _orient(OrientMeshs &meshs_,
            ...
            if (!params.parallel) {
                ...
                tbb::parallel_for(tbb::blocked_range<size_t>(0, meshs_.size()),
                                  [&meshs_, &params, progressfn, stopfn](
                                      ...
                                      for (size_t i = range.begin(); i != range.end(); ++i) {
                                          ...
                                          BOOST_LOG_TRIVIAL(debug) << "rotation_from_two_vectors: "
                                                                   ...
                                                                   << "; axis: "
                                                                   << mesh_.axis.transpose()
                                                                   << "; angle: " << mesh_.angle
                                                                   << "; euler: "
                                                                   << mesh_.euler_angles.transpose()
                                                                   << ", rotation_matrix:\n"
                                                                   << mesh_.rotation_matrix;
                                      }
                                  });
